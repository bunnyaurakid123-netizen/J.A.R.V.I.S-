...
Multi-layer AI brain · Auth · Persistent memory · Notes · Mode detection
"""
        ...
        with self._lock:
            ...
            if "\n" in text:
                ...
                for line in lines:
                    ...
                    if line:
                        try:
                            self._log.append_direct(f"{self._prefix} {line}")
                        except Exception:
                            pass
    ...
    def flush(self):
        with self._lock:
            ...
            if remaining:
                self._buf = io.StringIO()
                try:
                    self._log.append_direct(f"{self._prefix} {remaining}")
                except Exception:
                    pass
...
class JarvisApp:
    def __init__(self):
        ...
        try:
            ...
            self.system_prompt = "You are JARVIS. Be concise, professional and helpful."
        ...
        self.commander = CommandExecutor(
            tts_speak_fn=lambda text: self.tts.speak(text),
            log_fn=lambda text: self._log(text),
        )
        ...
        self._current_voice = "en-US-GuyNeural"
        ...
        self.window.send_text.connect(self._on_user_text)
        self.window.mic_clicked.connect(self._on_mic)
        self.window.stop_clicked.connect(self._on_stop)
        self.window.settings_changed.connect(self._on_settings_changed)
        self.window.note_saved.connect(self._on_note_saved)

        self._stderr = _LogRedirector(self.window.chat_display, "[ERR]")
        self._stdout = _LogRedirector(self.window.chat_display, "[LOG]")
        sys.stdout = self._stdout
        sys.stderr = self._stderr
        self._setup_tray()

        self._log("[SYS] J.A.R.V.I.S v3.0 starting...")
        self._log(f"[SYS] Model: {get_model()}")
    ...
    def _setup_tray(self):
        ...
        menu = QMenu()
        menu.addAction("Show JARVIS").triggered.connect(self.window.show)
    ...
    def _log(self, text: str):
        try:
            self.window.chat_display.append_direct(text)
        except Exception:
            pass

    def _check_connection(self):
        active = get_api_key()
        hint = f"...{active[-4:]}" if len(active) > 4 else "(configured locally)"
        ...
        self._log(f"[SYS] {'Gemini connected' if ok else 'Connection failed'} — {info}")

    def _on_user_text(self, text: str):
        if self._busy:
            self._log("[SYS] Please wait for the current response.")
            return
    ...
    def _on_mic(self):
        if self._busy:
            return
        self.window.set_mic_active(True)
        ...
        def _listen():
            try:
                ...
                import wave
                sample_rate = 16000
                audio = sd.rec(sample_rate * 8, samplerate=sample_rate, channels=1, dtype="int16")
                sd.wait()
                audio_data = np.squeeze(audio)
                if float(np.sqrt(np.mean(audio_data.astype(np.float64) ** 2))) < 100:
                    self._log("[SYS] No speech detected.")
                    return
                wav_io = io.BytesIO()
                ...
                if text:
                    QTimer.singleShot(0, lambda t=text: self._process_user_input(t))
            except Exception as e:
                self._log(f"[SYS] Mic error: {e}")
            finally:
                self.window.set_mic_active(False)
                if not self._busy:
    ...
    def _on_stop(self):
        self.tts.stop(); self._tts_queue.clear(); self._request_id += 1
        self._busy = False; self.window.set_state("ONLINE")
    ...
    def _on_note_saved(self, title: str, content: str):
        self._log(f"[SYS] {mem_store.save_note(title, content)}")

    def _display_command_response(self, response: str):
    ...
    def _process_user_input(self, text: str):
        ...
        if executed:
            self._display_command_response(response)
            return

        self._current_mode = detect_mode(text)
        self.window.set_mode(self._current_mode)
        ...
        def _ai_loop():
            full_response = ""
            try:
                for event in call_stream(messages=self.history, system_prompt=self.system_prompt):
                    if req_id != self._request_id: return
                    if event["type"] == "chunk":
                        full_response += event["text"]
                        self._accumulated_text = full_response
                        ...
                        self._accumulated_text = full_response
                        QTimer.singleShot(0, lambda rid=req_id: self._update_ai_display(rid, True))
                        if full_response: self.history.append({"role": "assistant", "content": full_response})
                    elif event["type"] == "error":
                        full_response = event["text"]; self._accumulated_text = full_response
                        QTimer.singleShot(0, lambda rid=req_id: self._update_ai_display(rid, True))
                        break
            except Exception as e:
                self._accumulated_text = f"Unexpected error: {e}"
                QTimer.singleShot(0, lambda rid=req_id: self._update_ai_display(rid, True))
            finally:
                if req_id == self._request_id:
                    self._busy = False
                    mem_store.save_history(self.history)
                    self.window.set_state("ONLINE")

        threading.Thread(target=_ai_loop, daemon=True).start()

    def _update_ai_display(self, req_id: int, final: bool = False):
        ...
        if not self._ai_response_started:
            ...
            self.window.chat_display.add_message("JARVIS", text, is_user=False)
        elif final and len(text) > self._displayed_len:
            self.window.chat_display.add_message("JARVIS", text[self._displayed_len:], is_user=False)
            self._displayed_len = len(text)

    def _quit(self):
        self.tts.stop(); self.app.quit()
